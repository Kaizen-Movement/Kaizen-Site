...
export function Hero({ content }: { content: HomepageHero }) {
  return (
    <section className="relative flex min-h-[92vh] items-center overflow-hidden border-b border-white/10 pt-24">
      {/* Ambient signature mark, oversized, faint, off-center */}
      <div className="pointer-events-none absolute -right-[15%] top-1/2 h-[900px] w-[900px] -translate-y-1/2 bg-seal-radial">
        <SealMark animate className="h-full w-full text-gold/60" />
      </div>

      <div className="relative mx-auto w-full max-w-[1440px] px-6 lg:px-10">
        <div className="max-w-2xl">
          <p className="eyebrow mb-6">Kaizen — 改善 — Continuous Refinement</p>
          <h1 className="font-display text-6xl leading-[0.98] tracking-tight text-bone md:text-7xl">
            {content.title}
          </h1>
          <p className="mt-6 font-display text-2xl italic text-bone/80">
            {content.subtitle}
          </p>
          {content.description ? (
